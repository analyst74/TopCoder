...
public:
	int count(vector <string> field, int dist) {
		return pow(2, getCombos(field, dist)) - 1;
	}
	
	int getCombos(vector <string> field, int dist) {
		int r = field.size(), c = field[0].size();
		vector <vector <bool> > explored(r, vector<bool>(c,false));
		int res = 0;
		...
		FOR (i, 0, r-1)
			FOR (j, 0, c-1) {
				if (!explored[i][j]) {
					if (field[i][j] == 'v') 
						res++;
					markExplored(field, dist, explored, i, j);
				}
			}
	...
	void markExplored(vector <string> field, int dist, vector <vector<bool> > explored, int i, int j) {
		int r = field.size(), c = field[0].size();
		
		for( std::vector<vector<bool> >::const_iterator a = explored.begin(); a != explored.end(); ++a)
    		std::cout << *a << ' ';
    
		if (!explored[i][j]) {
			explored[i][j] = true;
			if (field[i][j] == 'v') {
				int left = max(0, j-dist);
				int right = min(c-1, j+dist);
				int top = i;
				int bot = min(r-1, i+dist);
				
//				printf("lrtb: %d, %d, %d, %d", left, right, top, bot);
				FOR (x, top, bot)
					FOR (y, left, right) {
						if (inDistance(x,y,i,j,dist))
							markExplored(field, dist, explored, x, y);
					}
			}
		}
	}
	
	bool inDistance(int x1, int y1, int x2, int y2, int dist) {
